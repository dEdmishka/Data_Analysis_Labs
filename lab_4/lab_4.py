...
# Підготовка графіку та даних для загальної кількості осіб із вказаною кількістю дітей
plt.figure(figsize=(10, 6))

# Гістограма для загальної кількості осіб із вказаною кількістю дітей (зеленого кольору)
plt.hist(data['Children'], bins=20, color='green', alpha=0.5, label='Загальна к-сть осіб із вказаною кількість дітей', edgecolor='black')

# Підписи осей та заголовок
plt.title("Гістограма загальної кількості дітей")
...
# Обчислення коефіцієнта кореляції Пірсона
correlation = np.corrcoef(data['Children'], data['Cars'])
